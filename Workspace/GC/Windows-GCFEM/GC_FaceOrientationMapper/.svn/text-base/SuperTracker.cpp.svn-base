...
void SuperTracker::writeImages(char* filename){
	char fullfilename[100]="";
	CvRect *nose=new CvRect(),*temp;
	IplImage *tempImage,*tempImageNew=cvCreateImage(cvSize(120,100), IPL_DEPTH_8U, 3);
	/*strcpy(fullfilename,filename);
	temp=eyeDetector->GetImage(1);
	strcat(fullfilename,"e1.jpg\0");
	if(temp)cvSaveImage(fullfilename,temp);
	cvReleaseImage(&temp);
	temp=eyeDetector->GetImage(2);
	strcpy(fullfilename,filename);
	strcat(fullfilename,"e2.jpg\0");
	if(temp)cvSaveImage(fullfilename,temp);
	cvReleaseImage(&temp);
	temp=eyeRegionDetector->GetImage(1);
	strcpy(fullfilename,filename);
	strcat(fullfilename,"er.jpg\0");
	if(temp)cvSaveImage(fullfilename,temp);
	cvReleaseImage(&temp);
	temp=noseDetector->GetImage(1);
	strcpy(fullfilename,filename);
	strcat(fullfilename,"n.jpg\0");
	if(temp)cvSaveImage(fullfilename,temp);
	cvReleaseImage(&temp);*/

	tempImage=faceDetector->GetImage(1);
	strcpy(fullfilename,filename);
	strcat(fullfilename,"f.jpg\0");
	if(tempImage){
		cvResize(tempImage,tempImageNew);
		cvSaveImage(fullfilename,tempImageNew);
	}
	cvReleaseImage(&tempImage);
	cvReleaseImage(&tempImageNew);

	*nose=noseDetector->getLastRect();
	if(nose&& nose->x && nose->y && nose->width && nose->height){
		tempImageNew=cvCreateImage(cvSize(120,20), IPL_DEPTH_8U, 3);
		temp=new CvRect();
		temp->x=nose->x-nose->width;
		temp->y=nose->y;
		temp->width=nose->width*3;
		temp->height=nose->height;
		cvSetImageROI(frame,*temp);
		tempImage=(IplImage*)cvClone(frame);
		cvResetImageROI(frame);
		strcpy(fullfilename,filename);
		strcat(fullfilename,"H-NoseRegion.jpg\0");
		if(tempImage){
			cvResize(tempImage,tempImageNew);
			cvSaveImage(fullfilename,tempImageNew);
		}
		...
		tempImageNew=cvCreateImage(cvSize(120,40), IPL_DEPTH_8U, 3);
		temp->y=nose->y-nose->height*2;
		temp->height=nose->height*2;
		cvSetImageROI(frame,*temp);
		tempImage=(IplImage*)cvClone(frame);
		cvResetImageROI(frame);
		strcpy(fullfilename,filename);
		strcat(fullfilename,"H-EyeRegion.jpg\0");
		if(tempImage){
			cvResize(tempImage,tempImageNew);
			cvSaveImage(fullfilename,tempImageNew);
		}
		...
		temp->y=nose->y+nose->height;
		cvSetImageROI(frame,*temp);
		tempImage=(IplImage*)cvClone(frame);
		cvResetImageROI(frame);
		strcpy(fullfilename,filename);
		strcat(fullfilename,"H-MouthRegion.jpg\0");
		if(tempImage){
			cvResize(tempImage,tempImageNew);
			cvSaveImage(fullfilename,tempImageNew);
		}
	...
	//temp=mouthDetector->GetImage(1);
	//strcpy(fullfilename,filename);
	//strcat(fullfilename,"m.jpg\0");
	//if(temp)cvSaveImage(fullfilename,temp);
	//cvReleaseImage(&temp);
}